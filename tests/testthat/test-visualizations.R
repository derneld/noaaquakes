...
test_that("eq_clean_data handles missing months/days correctly", {
  ...
  cleaned <- eq_clean_data(na_data) [cite: 2]
...
test_that("geom_timeline renders without error", {
  cleaned <- eq_clean_data(test_data) [cite: 2]
...
test_that("geom_timeline_label handles top N earthquakes", {
  ...
  multi_data <- bind_rows(test_data, test_data %>% mutate(Mag = 8.0, `Location Name` = "BIG ONE"))
  cleaned <- eq_clean_data(multi_data) [cite: 2]

  p <- ggplot(cleaned, aes(x = DATE, label = LOCATION_NAME, magnitude = EQ_PRIMARY)) +
